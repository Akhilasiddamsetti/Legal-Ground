...
from legal_ground.retrieval.bm25 import tokenize
from legal_ground.retrieval.vectors import VectorIndex, reciprocal_rank_fusion
from legal_ground.security.access import Principal, can_access


class ChunkSearchEngine:
    # Reserve up to this many evidence slots for the strongest BM25 (lexical) hits so a
    # lexical query cannot lose them to dense-arm dilution during RRF. Symmetric and
    # query-driven (no per-document/term boosts) — preserves the no-overfit invariant.
    LEXICAL_FLOOR = 3
    ...
    def search(self, query: str, principal: Principal, top_k: int = 5) -> list[dict]:
        ...
        # Guarantee the strongest lexical hits survive fusion. A purely lexical query
        # (e.g. "customer pressure") otherwise risks losing its top BM25 matches when the
        # dense arm — which ranks every chunk — dilutes them below the top_k cutoff.
        lexical_floor = [chunk_id for chunk_id, _ in bm25_scored[: min(self.LEXICAL_FLOOR, top_k)]]
        ordered_ids: list[str] = []
        for chunk_id in [*lexical_floor, *(cid for cid, _ in fused)]:
            if chunk_id not in ordered_ids:
                ordered_ids.append(chunk_id)

        return [
            {"score": fused_scores.get(chunk_id, 0.0), "chunk": self.by_id[chunk_id]}
