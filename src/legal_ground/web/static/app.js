/* ============================================================
   Legal Ground — Case Workspace front-end
...
/* ---------- helpers ---------- */
/* ---------- per-case suggested questions ---------- */
function suggestionsForMatter(matterId) {
  const card = caseCards.find((c) => (c.dataset.matter || "") === matterId) || caseCards[0];
  if (!card) return [];
  try { return JSON.parse(card.dataset.suggestions || "[]"); } catch (_) { return []; }
}
function chipButton(s, useShort, className) {
  const b = document.createElement("button");
  b.type = "button";
  b.className = className;
  b.dataset.q = s.full;
  b.textContent = (useShort && s.short) ? s.short : s.full;
  b.addEventListener("click", () => { questionInput.value = s.full; autoGrow(); ask(s.full); });
  return b;
}
// Refresh the persistent "Quick ask" chips below the composer for the active case.
function renderQuickAsk() {
  const row = document.querySelector(".quick-ask");
  if (!row) return;
  row.querySelectorAll(".chip").forEach((c) => c.remove());
  suggestionsForMatter(currentMatter()).forEach((s) => row.appendChild(chipButton(s, true, "chip")));
}
...
function makeEmptyState(matterName) {
  ...
  // Rebuild the empty-state suggestion buttons for the active case.
  const row = e.querySelector(".suggest-row");
  if (row) {
    row.innerHTML = "";
    suggestionsForMatter(currentMatter()).forEach((s) => row.appendChild(chipButton(s, false, "suggest chip")));
  }
  return e;
}
