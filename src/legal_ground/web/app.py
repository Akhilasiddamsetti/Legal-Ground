...
import legal_ground.web.documents as documents_mod
from legal_ground import config
from legal_ground.web.runtime import (
    AppRuntime,
    build_live_runtime,
    build_principal,
...
# Web assets are bundled inside the package (legal_ground/web/{templates,static}),
...
# Fallback "quick ask" prompts, used only if a case has no per-case questions
# (each case normally supplies its own from _meta/demo-questions.md, attached to
# the case card by runtime.build_cases). The front-end swaps these per selected case.
DEFAULT_SUGGESTED_QUESTIONS = [
    ...
    {"full": "What key facts remain unresolved even after reviewing the full file?", "short": "Open issues"},
]


def _initial_suggestions(cases: list[dict]) -> list[dict]:
    """Questions for the first/default case, shown before JS swaps per selection."""
    if cases and cases[0].get("suggestions"):
        return cases[0]["suggestions"]
    return DEFAULT_SUGGESTED_QUESTIONS


# Friendly display names for known model ids; unknown ids fall back to the raw id.
_MODEL_DISPLAY = {
    "global.anthropic.claude-haiku-4-5-20251001-v1:0": "Claude Haiku 4.5 · Bedrock",
    "us.anthropic.claude-haiku-4-5-20251001-v1:0": "Claude Haiku 4.5 · Bedrock",
}


def model_display_name(model_id: str) -> str:
    """A human-friendly label for the active model; empty string hides it in the UI."""
    if not model_id:
        return ""
...
def create_app(runtime: AppRuntime) -> FastAPI:
    ...
    def index(request: Request) -> HTMLResponse:
        return TEMPLATES.TemplateResponse(
            ...
            "index.html",
            {
                "app_title": "Legal Ground — Case Workspace",
                "matters": runtime.allowed_matters,
                "roles": runtime.allowed_roles,
                "cases": runtime.cases,
                "documents": runtime.documents,
                "suggestions": _initial_suggestions(runtime.cases),
                "model_label": model_display_name(runtime.model_label),
                "user": {
                    "name": "Pilot User",
...
def main() -> None:
    ...
    print(
        f"[Legal Ground] Bedrock model: {runtime.model_label or '(none)'} "
        f"| region: {args.aws_region or '(unset)'}",
        flush=True,
    )
